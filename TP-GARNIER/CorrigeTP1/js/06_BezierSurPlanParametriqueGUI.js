...
let ptP0 = new THREE.Vector3(0, 0, 0);
let ptP1 = new THREE.Vector3(0, 0, 0);
let ptP2 = new THREE.Vector3(0, 0, 0);
...
function PtsCourbeBezier2(nb,vecN,vec_e1){
  let points = new Array(nb+1);
  let P0= new THREE.Vector2(1,1);
  let P1= new THREE.Vector2(0,2);
  let P2= new THREE.Vector2(-2,-2);
  for(var k=0;k<=nb;k++){
    let t2=k/nb;
    let x0=Math.pow(1-t2,2)*P0.x+2*t2*(1-t2)*P1.x+Math.pow(t2,2)*P2.x;
    let y0=Math.pow(1-t2,2)*P0.y+2*t2*(1-t2)*P1.y+Math.pow(t2,2)*P2.y;
    vecN.normalize();
    vec_e1.normalize();
    vec_e2.crossVectors(vecN,vec_e1);
    let v1=vec_e1.clone();
  ...
  let V1=vec_e1.clone();
  let V2=vec_e2.clone();
  V1.multiplyScalar(P0.x);
  V2.multiplyScalar(P0.y);
  ptP0.addVectors(V1,V2);
  ptP0.addVectors(ptA,ptP0);
  V1=vec_e1.clone();
  V2=vec_e2.clone();
  V1.multiplyScalar(P1.x);
  V2.multiplyScalar(P1.y);
  ptP1.addVectors(V1,V2);
  ptP1.addVectors(ptA,ptP1);
  V1=vec_e1.clone();
  V2=vec_e2.clone();
  V1.multiplyScalar(P2.x);
  V2.multiplyScalar(P2.y);
  ptP2.addVectors(V1,V2);
  ptP2.addVectors(ptA,ptP2);

  mes2d='X(t) = (1-t)<span class="expo">2</span>&times'+P0.x;
  mes2d+="+ 2 &times t &times (1-t)&times"+P1.x;
  mes2d+='+ t<span class="expo">2</span>&times'+P2.x+" <br />";
  mes2d+='Y(t) = (1-t)<span class="expo">2</span>&times'+P0.y;
  mes2d+="+ 2 &times t &times (1-t)&times"+P1.y;
  mes2d+='+ t<span class="expo">2</span>&times'+P2.y
  document.getElementById("Eq2d").innerHTML=mes2d;


  return PtsCbePara;
}//fin function PtsCourbeBezier2
...
//ajout
 ...
 let epaiCbe = 3;
 let dimPt=0.05;
 let PtsTab = PtsCourbeBezier2(nb,vecN,vec_e1);
 ...
 let pointP0 = tracePt(scene, ptP0, 0x00FF00,dimPt,false);
 let pointP1 = tracePt(scene, ptP1, 0x0000FF,dimPt,false);
 let pointP2 = tracePt(scene, ptP2, 0x00FF00,dimPt,false);
 scene.add(pointA);
 scene.add(pointP0);
 scene.add(pointP1);
 scene.add(pointP2);
 ...
 scene.add(vecteurE1);
 scene.add(vecteurE2);
 // partie GUI 0.25, 0.125
    // initialisation des controles gui
 //var gui = new dat.GUI({ autoPlace: true });//interface graphique utilisateur
 ...
 ************************************************************/
 ...
 guiPlanGeom.add(menuGUI,'coef_a',-5,5).onChange(function(){
   ...
   PtsTab = PtsCourbeBezier2(nb,vecN,vec_e1);
   ...
   scene.add(vecteurE1);
   scene.add(vecteurE2);
   if (pointP0) scene.remove(pointP0);
   if (pointP1) scene.remove(pointP1);
   if (pointP2) scene.remove(pointP2);
   pointP0 = tracePt(scene, ptP0, 0x00FF00,dimPt,false);
   pointP1 = tracePt(scene, ptP1, 0x0000FF,dimPt,false);
   pointP2 = tracePt(scene, ptP2, 0x00FF00,dimPt,false);
   scene.add(pointP0);
   scene.add(pointP1);
   scene.add(pointP2);
 }); 
